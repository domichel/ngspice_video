...
2) is to get a practical to use result when some ngspice parametric ngspice simulation just gives too many plots.
files that it is not practical to compare them.

Making a video allow us to navigate into the result as we choose.

To be work and be useful, we use a Simulation.cmd file that run the parametric simulations, change the titles of the plots and show us what parameters was used for each single plot.
...
The index into Simulation.cmd beg1n at 1000. The plot title begin at 1. To know which file correspond ro a title plot, do <plot_title_number> + 999.
...
will do the same identical videos with a length of almost 25 minutes.
	./makevideos 6
may be or not a good compromise between readabilty and video length.
